      ...
      {steps.map((step, index) => (
        <div key={step.id} className="flex items-center">
          ...
          {index < steps.length - 1 && (
            <div
              className={`ml-[20px] mr-[20px] h-[1px] w-[40px]
                            ${
                              currentStep > step.id
                                ? 'bg-gradient-to-r from-[#65BAFA] to-[#9D37F2]'
                                : 'bg-gray-300'
                            }`}
            />
          )}
