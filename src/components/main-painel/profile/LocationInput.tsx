...
export function LocationInput({ cityData, stateData, handleLocation, enableForm }: LocationInputProps) {
    ...
    return (
        <div className=''>
            ...
            <LabelItem title="Localização" enableForm={enableForm} className='w-full'>
                {liveOutside ? (
                    ...
                    <div className='px-6 flex w-full'>
                        ...
                        >
                            <Select.Trigger className='py-4 pr-4 group flex gap-12 items-center justify-between'>
                                <Select.Value />
                                <Select.Icon>
                            ...
                            <Select.Content side='bottom' sideOffset={16} position='popper' className='w-full bg-white rounded p-2 shadow'>
                                <Select.Viewport className='max-h-32'>
                                    {statesOfBrazil?.map((state) => (
                                        <Select.Item
                                            className='p-3 cursor-pointer text-violet-500 hover:text-white hover:bg-violet-500 flex gap-4 justify-between rounded-md'
                                            key={state.sigla}
                                            value={state.sigla}
                        ...
                        >
                            <Select.Trigger className='py-4 pl-4 flex-1 group flex items-center justify-between'>
                                <Select.Value />
                                <Select.Icon>
                                    <CaretDown weight='fill' className='text-violet-500 group-data-[disabled]:opacity-20' />
                                </Select.Icon>
            ...
            <label htmlFor='checkbox' className='mt-5 flex gap-2'>
                <Checkbox.Root
                    className='w-6 h-6 flex gap-2 items-center border border-solid border-gray-400 bg-gray-300/20 data-[state=checked]:border-none data-[state=checked]:bg-gradient-to-r data-[state=checked]:from-blue-300 data-[state=checked]:to-violet-400 rounded'
                    disabled={!enableForm}
                    checked={liveOutside}
                ...
                >
                    <Checkbox.Indicator className=''>
                        <Check size={24} weight='bold' className='text-white p-[2px]' />
                    </Checkbox.Indicator>
                </Checkbox.Root>
                Não moro no Brasil
            </label>
