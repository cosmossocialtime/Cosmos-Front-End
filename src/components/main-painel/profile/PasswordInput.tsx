...
export function PasswordInput({ enableForm }: PasswordInputProps) {
  ...
  return (
    <div className="flex w-full justify-between px-6 py-4">
      ...
      <Dialog.Root>
        <Dialog.Trigger>
          <span className="font-semibold text-blue-400">Alterar senha</span>
        </Dialog.Trigger>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/25" />
          <Dialog.Content className="fixed left-1/2 top-1/2 flex w-[34rem] -translate-x-1/2 -translate-y-1/2 flex-col gap-6 rounded-lg bg-white px-16 py-[4.5rem]">
            <Dialog.Close>
            ...
            <button className="w-full rounded-lg bg-violet-500 py-4 text-lg font-semibold text-white transition-colors hover:bg-violet-600">
              Alterar senha
            </button>
          </Dialog.Content>
        </Dialog.Portal>
